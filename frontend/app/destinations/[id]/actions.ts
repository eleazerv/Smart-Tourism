...
import { ApiError, createReview, deleteReview } from "@/lib/api";
...
/** The API rejects anything outside this range before it reaches the table. */
...
function explain(error: unknown, fallback: string): ReviewResult {
  ...
  if (error.status === 409) {
    return { ok: false, message: "Anda sudah pernah mengulas destinasi ini." };
  }
...
export async function submitReview(
  ...
  try {
    await createReview(destinationId, { rating, comment, photo }, { token });
  } catch (error) {
