/** Placeholder held by the Suspense boundary on `/hotels` and `/flights`. */

function Block({ className }: { className: string }) {
  return <div className={`animate-pulse rounded-2xl bg-muted ${className}`} />;
}

export function ListingSkeleton({ rows = 5 }: { rows?: number }) {
  return (
    <>
      <div className="bg-brand-900/90">
        <div className="container-page space-y-3 py-10 sm:py-14">
          ...
          <Block className="h-9 w-80 max-w-full rounded-md bg-brand-700" />
          <Block className="h-4 w-96 max-w-full rounded-md bg-brand-700" />
          <Block className="mt-4 h-20 w-full rounded-2xl bg-brand-700" />
        </div>
      </div>
      ...
      <div className="container-page grid gap-8 py-8 lg:grid-cols-[16rem_1fr]">
        ...
        <div className="space-y-4">
          ...
          {Array.from({ length: rows }, (_, i) => (
            <Block key={i} className="h-48 sm:h-44" />
          ))}
        </div>
      </div>
    </>
