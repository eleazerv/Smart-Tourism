...
export function AccommodationBookingList() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!token) return;
      try {
        setBookings(await listAccommodationBookings({ token }));
      } catch {
        setError(true);
      }
    })();
  ...
  return (
    <ul className="space-y-3">
      {bookings.map((booking) => {
        const firstRoom = booking.accommodation_booking_rooms[0];
        const name = firstRoom?.accommodations?.name ?? booking.booking_code;
