...
<div align="center">
  ...
  <p><em>Homepage — Eksplorasi destinasi unggulan di luar Bali</em></p>

  <img src="docs/screenshots/recommendations.png" alt="Rekomendasi" width="800"/>
  <p><em>Rekomendasi Destinasi — Hasil yang dipersonalisasi berdasarkan preferensi pengguna</em></p>

  <img src="docs/screenshots/trip-planner.png" alt="Trip Planner" width="800"/>
  <p><em>Trip Planner — Menyusun perjalanan multi-kota lengkap dengan akomodasi dan penerbangan</em></p>

  <img src="docs/screenshots/budget.png" alt="Estimasi Budget" width="800"/>
  <p><em>Estimasi Budget — Rincian perkiraan biaya perjalanan</em></p>
...
```sql
-- ============================================================================
-- JELANTARA — DATABASE SCHEMA EXPORT
-- Dikelompokkan per kategori — Sat Sep  5 00:30:11 SEAST 2026
-- ============================================================================
...
-- accommodation_booking_rooms
create policy accommodation_booking_rooms_read_own on public.accommodation_booking_rooms
  as permissive for select to authenticated
  using (EXISTS (
    SELECT 1 FROM accommodation_bookings b
    WHERE b.id = accommodation_booking_rooms.booking_id AND b.user_id = auth.uid()
  ));

-- accommodation_bookings
create policy "own acc bookings" on public.accommodation_bookings
  as permissive for all to public
  using (auth.uid() = user_id);

-- accommodation_reviews
create policy accommodation_reviews_read on public.accommodation_reviews
  as permissive for select to public
  using (true);

create policy accommodation_reviews_write on public.accommodation_reviews
  as permissive for insert to public
  with check (user_id = auth.uid());

create policy accommodation_reviews_delete on public.accommodation_reviews
...
-- airports
create policy airports_read_all on public.airports
  as permissive for select to authenticated, anon
  using (true);

-- budget_estimates
create policy "own estimates" on public.budget_estimates
  as permissive for all to public
  using (auth.uid() = user_id);
...
-- chat_rooms
create policy chat_rooms_own on public.chat_rooms
  as permissive for all to public
  using (user_id = auth.uid())
  with check (user_id = auth.uid());
...
-- climate_pattern_tags
create policy "read climate_pattern_tags" on public.climate_pattern_tags
  as permissive for select to public
  using (true);

-- climate_patterns
create policy "read climate" on public.climate_patterns
  as permissive for select to public
  using (true);

-- destination_tags
create policy "read destination_tags" on public.destination_tags
  as permissive for select to public
  using (true);

-- destination_views
create policy "read destination_views" on public.destination_views
  as permissive for select to public
  using (true);

create policy "anyone can track view" on public.destination_views
  as permissive for insert to public
  with check (true);
...
-- flight_booking_items
create policy "own flight items" on public.flight_booking_items
  as permissive for all to public
  using (EXISTS (
...
-- flight_options
create policy "read flights" on public.flight_options
  as permissive for select to public
  using (true);

-- flight_seats
create policy flight_seats_read_all on public.flight_seats
  as permissive for select to authenticated, anon
  using (true);

-- flight_tickets
create policy flight_tickets_own on public.flight_tickets
  as permissive for all to public
  using (EXISTS (
    SELECT 1 FROM flight_bookings fb
...
-- pricing_baseline
create policy "read pricing" on public.pricing_baseline
  as permissive for select to public
  using (true);
...
-- review_likes
create policy "read review_likes" on public.review_likes
  as permissive for select to public
  using (true);

create policy "insert own like" on public.review_likes
  as permissive for insert to public
  with check (auth.uid() = user_id);

create policy "delete own like" on public.review_likes
  as permissive for delete to public
  using (auth.uid() = user_id);

create policy "users can unlike own likes" on public.review_likes
...
-- trip_bookings
create policy "own trip bookings" on public.trip_bookings
  as permissive for all to public
  using (auth.uid() = user_id);

-- trip_flights
create policy trip_flights_own on public.trip_flights
  as permissive for all to public
  using (EXISTS (
...
-- trip_stops
create policy trip_stops_own on public.trip_stops
  as permissive for all to public
  using (EXISTS (
...
-- user_preference_tags
create policy "own preference tags" on public.user_preference_tags
  as permissive for all to public
  using (auth.uid() = user_id);
...
create policy "read public profile" on public.users
  as permissive for select to public
  using (true);
...
-- visitor_stats
create policy "read visitor_stats" on public.visitor_stats
  as permissive for select to public
  using (true);


-- ----------------------------------------------------------------------------
-- Status RLS per tabel (aktif / jumlah policy)
-- ----------------------------------------------------------------------------
-- accommodation_booking_rooms : aktif, 1 policy
-- accommodation_bookings      : aktif, 1 policy
-- accommodation_reviews       : aktif, 3 policy
-- accommodations              : aktif, 1 policy
-- airports                    : aktif, 1 policy
-- budget_estimates            : aktif, 1 policy
-- chat_messages               : aktif, 1 policy
-- chat_rooms                  : aktif, 1 policy
-- cities                      : aktif, 1 policy
-- climate_pattern_tags        : aktif, 1 policy
-- climate_patterns            : aktif, 1 policy
-- destination_tags            : aktif, 1 policy
-- destination_views           : aktif, 2 policy
-- destinations                : aktif, 1 policy
-- events                      : aktif, 1 policy
-- flight_booking_items        : aktif, 1 policy
-- flight_bookings             : aktif, 1 policy
-- flight_options               : aktif, 1 policy
-- flight_seats                : aktif, 1 policy
-- flight_tickets              : aktif, 1 policy
-- pricing_baseline            : aktif, 1 policy
-- provinces                   : aktif, 1 policy
-- review_likes                : aktif, 4 policy
-- reviews                     : aktif, 4 policy
-- saved_destinations          : aktif, 1 policy
-- tags                        : aktif, 1 policy
-- trip_bookings               : aktif, 1 policy
-- trip_flights                : aktif, 1 policy
-- trip_items                  : aktif, 1 policy
-- trip_stops                  : aktif, 1 policy
-- trips                       : aktif, 2 policy
-- user_preference_tags        : aktif, 1 policy
-- users                       : aktif, 3 policy
-- visitor_stats                : aktif, 1 policy
...
CREATE OR REPLACE FUNCTION public.search_destinations(
  ...
  tag_slugs text[] DEFAULT NULL::text[],
  filter_province_id integer DEFAULT NULL::integer,
  filter_city_id integer DEFAULT NULL::integer,
...
-- ----------------------------------------------------------------------------
-- Extension pg_trgm (bawaan, tidak perlu dibuat ulang)
-- ----------------------------------------------------------------------------
...
-- gtrgm_picksplit, gtrgm_same, gtrgm_union, set_limit, show_limit, show_trgm,
-- similarity, similarity_dist, similarity_op, strict_word_similarity,
...
CREATE TRIGGER trg_update_accommodation_rating
  AFTER INSERT OR DELETE OR UPDATE ON public.accommodation_reviews
...
CREATE TRIGGER trg_default_avatar
  ...
  FOR EACH ROW EXECUTE FUNCTION generate_default_avatar();
```
...
jelantara-backend/
├── router/
│   ├── auth.Route.js                    # Registrasi, login, sesi pengguna
│   ├── destinations.Route.js            # CRUD & pencarian destinasi
│   ├── cities.Route.js                  # Direktori kota
│   ├── events.Route.js                  # Event & festival
│   ├── tags.Route.js                    # Kategori minat
│   ├── heatmap.Route.js                 # Persebaran destinasi geografis
│   ├── recommendations.Route.js         # Rekomendasi personal
│   ├── preferences.Route.js             # Preferensi pengguna
│   ├── budget.Route.js                  # Estimasi biaya perjalanan
│   ├── accommodation.Route.js           # Katalog akomodasi
│   ├── flight.Route.js                  # Katalog penerbangan
│   ├── trip.Route.js                    # Trip & trip stops
│   ├── Route.Route.js                   # Pencarian rute perjalanan
│   ├── saved-destinations.Route.js      # Destinasi tersimpan
│   ├── accomodationBooking.Route.js     # Pemesanan akomodasi
│   ├── flightbooking.Route.js           # Pemesanan penerbangan
│   ├── tripbooking.Route.js             # Checkout satu invoice per trip
│   ├── chat.Route.js                    # Chat assistant
│   └── webhook.Route.js                 # Callback pembayaran Xendit
├── lib/
│   └── bookingPayment.js                # Logika invoice & sapuan booking kedaluwarsa
├── test/
│   ├── unit/                            # Unit test (node:test) untuk fungsi murni di lib/
│   ├── integration/                     # Integration test lewat fake Supabase client in-memory
│   └── helpers/                         # fakeSupabase.js, dsb
├── test-all-endpoints.js                # E2E smoke test (taruh manual, lihat bagian Testing)
├── swagger.js                           # Konfigurasi dokumentasi API
├── main.js                              # Entry point aplikasi Express
├── .env.example                         # Contoh variabel lingkungan
└── package.json
```

> Skema database (tabel, RLS, function) belum punya script migrasi/seed otomatis — lihat langkah manual di bagian "4️⃣ Setup Database" di bawah.

### Folder Structure (Frontend)

Next.js 15 (App Router) + React 19 + TypeScript, styling Tailwind CSS + komponen berbasis Radix UI (pola shadcn/ui).

```
jelantara-frontend/
├── app/                                  # Routing (App Router) — tiap folder = satu halaman
│   ├── page.tsx                          # Homepage
│   ├── layout.tsx                        # Root layout
│   ├── destinations/                     # Katalog & detail destinasi ([id])
│   ├── hotels/                           # Katalog & detail akomodasi ([id])
│   ├── flights/                          # Pencarian & pemesanan penerbangan (pesan/)
│   ├── rencana/                          # Trip planner (AI + panel langsung)
│   ├── recommendations/                  # Rekomendasi musiman
│   ├── peta/                             # Heatmap & peta persebaran destinasi
│   ├── akun/                             # Profil, minat, pesanan, kata sandi
│   ├── auth/                             # Login, sign-up, lupa/reset password
│   └── api/                              # Route handler internal (destination-season, flight-calendar)
├── components/                           # Komponen UI, dikelompokkan per domain
│   ├── home/                             # Hero, rail destinasi, header/footer
│   ├── destination/ , destinations/      # Detail satu destinasi vs halaman katalog
│   ├── flights/ , stays/                 # Pencarian & filter penerbangan / akomodasi
│   ├── planner/                          # Chat AI, pemilih penerbangan, kartu interaktif
│   ├── account/                          # Sidebar akun, booking, preferensi, avatar
│   ├── auth/                             # Shell form login/sign-up
│   ├── peta/                             # Peta crowd-level & lokasi
│   ├── recommendations/                  # Season board & kartu waktu kunjungan
│   ├── catalogue/                        # Filter, pagination, search bar (dipakai lintas halaman)
│   └── ui/                               # Primitif dasar (button, card, dropdown, dsb — pola shadcn/ui)
├── lib/
│   ├── api/                              # Client fetch ke backend Express (client.ts, endpoints.ts, types.ts)
│   ├── supabase/                         # Klien Supabase sisi client/server + proxy auth
│   └── *.ts                              # Helper per fitur (format-price, seat-map, calendar, dsb)
├── public/
│   └── geo/                              # GeoJSON batas provinsi (dipakai peta)
├── next.config.ts
├── tailwind.config.ts
